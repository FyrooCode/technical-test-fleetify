...
export const InvoicePrintPreview: React.FC<InvoicePrintPreviewProps> = ({ invoice }) => {
  return (
      ...
      <div style={{ fontFamily: 'Arial, sans-serif', color: '#000', lineHeight: '1.5' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '2px solid #000', paddingBottom: '16px', marginBottom: '24px' }}>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <p style={{ margin: 0, fontSize: '14px', fontWeight: 'bold', color: '#4b5563' }}>{invoice.invoice_number}</p>
            <p style={{ margin: 0, fontSize: '14px', color: '#6b7280' }}>
              Tanggal: {new Date(invoice.created_at).toLocaleDateString('id-ID')} Jam: {new Date(invoice.created_at).toLocaleTimeString('id-ID')}
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '32px' }}>
          <div style={{ width: '45%' }}>
            ...
            <p style={{ margin: '0 0 4px 0', fontWeight: 'bold', fontSize: '16px' }}>{invoice.receiver_name}</p>
            <p style={{ margin: 0, fontSize: '14px' }}>{invoice.receiver_address}</p>
          </div>
          <div style={{ width: '45%' }}>
            <h3 style={{ fontSize: '12px', textTransform: 'uppercase', color: '#6b7280', margin: '0 0 8px 0' }}>From (Pengirim):</h3>
            <p style={{ margin: '0 0 4px 0', fontWeight: 'bold', fontSize: '16px' }}>{invoice.sender_name}</p>
            <p style={{ margin: 0, fontSize: '14px' }}>{invoice.sender_address}</p>
          </div>
        </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '32px' }}>
          ...
          <tbody>
            {invoice.details.map((item: InvoiceDetailResponse, idx: number) => (
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '48px' }}>
          <div style={{ width: '300px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontSize: '14px' }}>
              ...
              <span>Rp {new Intl.NumberFormat('id-ID').format(invoice.total_amount)}</span>
            </div>
